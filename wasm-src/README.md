...
| File | Constant in `index.html` (or the `src/` file it moved to) | Exports | Used by (JS fallback exists) |
|---|---|---|---|
...
1. Edit the `.wat` file.
2. `node tools/wasm-sources.cjs --build` and replace the matching constant in `index.html` (or the `src/` file it moved to). The rebuilt module has no custom sections, which is fine for execution.
